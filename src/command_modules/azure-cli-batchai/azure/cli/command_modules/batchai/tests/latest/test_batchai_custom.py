...
class TestBatchAICustom(unittest.TestCase):
    ...
    def test_batchai_patch_mount_volumes_no_volumes(self):
        self.assertIsNone(_patch_mount_volumes(MockCli(), None))

    def test_batchai_patch_mount_volumes_empty_volumes(self):
        self.assertEqual(MountVolumes(), _patch_mount_volumes(MockCli(), MountVolumes()))
    ...
    def test_batchai_patch_mount_volumes_with_templates_via_command_line_args(self):
        ...
        actual = _patch_mount_volumes(MockCli(), mount_volumes, 'account', 'key')
    ...
    def test_batchai_patch_mount_volumes_with_templates_via_env_variables(self):
        ...
        with _given_env_variable('AZURE_BATCHAI_STORAGE_ACCOUNT', 'account'):
            with _given_env_variable('AZURE_BATCHAI_STORAGE_KEY', 'key'):
                actual = _patch_mount_volumes(MockCli(), mount_volumes)
        expected = MountVolumes(
            azure_file_shares=[
                AzureFileShareReference(
                    relative_mount_path='azfiles',
    ...
    def test_batchai_patch_mount_volumes_with_credentials_no_account_found(self, get_storage_client):
        ...
        with self.assertRaisesRegexp(CLIError, 'Cannot find "account1" storage account'):
            _patch_mount_volumes(MockCli(), mount_volumes)
    ...
    def test_batchai_add_nfs_to_empty_mount_volumes(self):
        ...
        self.assertEquals('id', mount_volumes.file_servers[0].file_server.id)
        self.assertEquals('relative_path', mount_volumes.file_servers[0].relative_mount_path)
        self.assertEquals('rw', mount_volumes.file_servers[0].mount_options)
    ...
    def test_batchai_add_nfs_to_non_empty_mount_volumes(self):
        ...
        for i in range(2):
            self.assertEquals('id{0}'.format(i), mount_volumes.file_servers[i].file_server.id)
            self.assertEquals('relative_path{0}'.format(i), mount_volumes.file_servers[i].relative_mount_path)
            self.assertEquals('rw', mount_volumes.file_servers[i].mount_options)

    def test_batchai_add_azure_file_share_to_mount_volumes_no_account_info(self):
        with self.assertRaisesRegexp(CLIError, 'Please configure Azure Storage account name'):
            _add_azure_file_share_to_mount_volumes(MockCli(), MountVolumes(), 'share', 'relative_path')
    ...
    def test_batchai_add_azure_file_share_to_mount_volumes_no_relative_mount_path(self):
        with self.assertRaisesRegexp(CLIError, 'Azure File share relative mount path cannot be empty'):
            _add_azure_file_share_to_mount_volumes(MockCli(), MountVolumes(), 'share', '')
    ...
    def test_batchai_add_azure_file_share_to_mount_volumes_account_and_key_via_env_variables(
        ...
        with _given_env_variable('AZURE_BATCHAI_STORAGE_ACCOUNT', 'account'):
            with _given_env_variable('AZURE_BATCHAI_STORAGE_KEY', 'key'):
                actual = _add_azure_file_share_to_mount_volumes(MockCli(), MountVolumes(), 'share', 'relative_path')
    ...
    def test_batchai_add_azure_file_share_to_mount_volumes_account_via_command_line_args(self, get_storage_client):
        get_storage_client.return_value = _get_mock_storage_accounts_and_keys({'account': 'key'})
        actual = _add_azure_file_share_to_mount_volumes(MockCli(), MountVolumes(), 'share', 'relative_path', 'account')
        expected = MountVolumes(azure_file_shares=[
            AzureFileShareReference(
                account_name='account',
                azure_file_url='https://account.file.core.windows.net/share',
    ...
    def test_batchai_add_azure_file_share_to_absent_mount_volumes(self, get_storage_client):
        get_storage_client.return_value = _get_mock_storage_accounts_and_keys({'account': 'key'})
        actual = _add_azure_file_share_to_mount_volumes(MockCli(), None, 'share', 'relative_path', 'account')
        expected = MountVolumes(azure_file_shares=[
            AzureFileShareReference(
                account_name='account',
                azure_file_url='https://account.file.core.windows.net/share',
    ...
    def test_batchai_add_azure_container_mount_volumes_no_account(self):
        with self.assertRaisesRegexp(CLIError, 'Please configure Azure Storage account name'):
            _add_azure_container_to_mount_volumes(MockCli(), MountVolumes(), 'container', 'relative_path')
    ...
    def test_batchai_add_azure_container_mount_volumes_no_relative_path(self):
        with self.assertRaisesRegexp(CLIError, 'Azure Storage Container relative mount path cannot be empty'):
            _add_azure_container_to_mount_volumes(MockCli(), MountVolumes(), 'container', '')

    def test_batchai_add_azure_container_to_absent_mount_volumes(self):
        with _given_env_variable('AZURE_BATCHAI_STORAGE_ACCOUNT', 'account'):
            with _given_env_variable('AZURE_BATCHAI_STORAGE_KEY', 'key'):
                actual = _add_azure_container_to_mount_volumes(MockCli(), None, 'container', 'relative_path')
    ...
    def test_batchai_add_azure_container_mount_volumes_account_and_key_via_env_variables(self):
        with _given_env_variable('AZURE_BATCHAI_STORAGE_ACCOUNT', 'account'):
            with _given_env_variable('AZURE_BATCHAI_STORAGE_KEY', 'key'):
                actual = _add_azure_container_to_mount_volumes(MockCli(), MountVolumes(), 'container', 'relative_path')
        expected = MountVolumes(azure_blob_file_systems=[
            AzureBlobFileSystemReference(
                account_name='account',
                container_name='container',
    ...
    def test_batchai_add_azure_container_mount_volumes_account_and_key_via_cmd_line_args(self):
        actual = _add_azure_container_to_mount_volumes(
            MockCli(), MountVolumes(), 'container', 'relative_path', 'account', 'key')
        expected = MountVolumes(azure_blob_file_systems=[
            AzureBlobFileSystemReference(
                account_name='account',
                container_name='container',
    ...
    def test_batchai_add_azure_container_mount_volumes_account_via_cmd_line_args(self, get_storage_client):
        get_storage_client.return_value = _get_mock_storage_accounts_and_keys({'account': 'key'})
        actual = _add_azure_container_to_mount_volumes(
            MockCli(), MountVolumes(), 'container', 'relative_path', 'account')
        expected = MountVolumes(azure_blob_file_systems=[
            AzureBlobFileSystemReference(
                account_name='account',
                container_name='container',
    ...
    def test_list_setup_task_files_for_cluster_when_no_node_setup(self):
        self.assertEqual(_list_node_setup_files_for_cluster(MockCli(), Cluster(), '.', 60), [])
    ...
    def test_list_setup_task_files_for_cluster_when_output_not_on_mounts_root(self):
        with self.assertRaisesRegexp(CLIError, 'List files is supported only for clusters with startup task'):
            ...
            _list_node_setup_files_for_cluster(MockCli(), cluster, '.', 60)
